...
export function SessionView({ session, backTo, backLabel = '← Back' }) {
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-end justify-between gap-4">
        ...
        {backTo ? (
          <Link to={backTo} className="text-xs uppercase tracking-widest2 text-gold">{backLabel}</Link>
        ) : null}
      </header>
        ...
        <ul className="space-y-4">
          {exercises.map((ex, i) => {
            ...
            const bestSet = doneSets.reduce((best, s) => {
              ...
              return !best || w > best.weight
                ? { weight: w, reps: Number(s.reps) || 0, rpe: s.rpe }
                : best;
            }, null);
